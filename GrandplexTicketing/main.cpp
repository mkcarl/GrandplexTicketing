...
void print_title(string text)
{
	...
	if (text.size() % 2 == 0)
		cout << left << setw((LENGTH - text.size()) / 2) << "*" << text << right << setw((LENGTH - text.size()) / 2) <<
			"*" << endl;
	else
		cout << left << setw((LENGTH - text.size()) / 2) << "*" << text << right << setw((LENGTH - text.size()) / 2 + 1)
			<< "*" << endl;
	cout << string(LENGTH, '*') << endl;
}
			 ...
			 */
			{
				...
				do
				{
					...
					while (true)
					{
						...
						{
							cout << "Invalid rating value input, please try again with a value between 1 to 5 ! " <<
								endl;
							continue;
						}
						break;
					...
					while (true)
					{
						...
						if (newMovie.air_hour > 23 || newMovie.air_hour < 0 && newMovie.air_min < 0 || newMovie.air_min
							> 59)
						{
							cout <<
								"Invalid time format input, please check and try again ! Press any key to continue..."
								<< endl << endl;
							continue;
							//no input validation for if "string" is done.
						}
						string combTime = to_string(newMovie.air_hour) + to_string(newMovie.air_min);
						cout << "Time selected is " << setfill('0') << setw(2) << newMovie.air_hour << setw(2) <<
							newMovie.air_min << " hours" << endl << setfill(' ');
		...
		case 5:
			 ...
			 */
			{
				...
				while (true)
				{
					...
					{
						cout << "Invalid choice! Please type numbers between 1 and " << movie_list.getSize() << " only!"
							<< endl << endl;
						continue;
					}
			 ...
			 */
			{
				...
				do
				{
					...
					while (true)
					{
						...
						if (movChoice_i <= 0 || movChoice_i > movie_list.getSize())
						{
							cout << "Invalid choice! Please type numbers between 1 and " << movie_list.getSize() <<
								" only!" << endl << endl;
							continue;
						}
